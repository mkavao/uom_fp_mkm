use "F:\Dropbox_APHRC\Dropbox\FP-Project-APHRC-UM\Atelier_FP\Data\Donnees_ucad\hf_data_combined.dta",clear

drop SubmissionDate	start end today s1_7	s1_8	s1_9	s1_10	s2_1

lab var s3_1_1_1  "Waiting room with seats"
lab var s3_1_2_1  "Men toilet with running water"
lab var s3_1_3_1  "Women toilet with running water"
lab var s3_1_4_1  "Handwash facilities"
lab var s3_1_5_1  "Drinking water"
lab var s3_1_6_1  "Electricity supply"
lab var s3_1_8_1  "Laboratory"
lab var s3_1_9_1  "Operating room"
lab var s3_1_10_1 "Pharmacy"
lab var s3_1_11_1 "Signage for orientation"
lab var s3_1_12_1 "Ramps for disabled people"
lab var s3_1_13_1 "Biomedical waste collection"
lab var s3_1_14_1 "Car park"

recode  s3_1_?_1 s3_1_??_1 (2=0) (.=0)
****
* Define your binary variables
local binary_vars "s3_1_1_1	s3_1_2_1	s3_1_3_1	s3_1_4_1	s3_1_5_1	s3_1_6_1	s3_1_8_1	s3_1_9_1	s3_1_10_1	s3_1_11_1	s3_1_12_1	s3_1_13_1	s3_1_14_1" // Replace with your variable names

* Initialize Excel file
putexcel set "Proportions_Summary.xlsx", sheet("Sheet1") replace

* Write headers
putexcel A1:F1 = "General infrastructure (% of facilities)",bold merge hcenter border(bottom)
putexcel A2 = "Infrastructure" B2 = "EPS"  c2 = "CS" d2 = "PS" e2 = "Case de Sante" f2 = "Clinique",bold border(bottom,double)

* Calculate proportions and export to Excel
local row = 3 // Start writing from row 2
foreach var of local binary_vars {
    quietly summarize `var' if hf_type==1 // Calculate mean (proportion of 1s)
    local mean1 = r(mean)*100    // Store the mean
    
	    quietly summarize `var' if hf_type==2 // Calculate mean (proportion of 1s)
    local mean2 = r(mean)*100    // Store the mean

	
	    quietly summarize `var' if hf_type==3 // Calculate mean (proportion of 1s)
    local mean3 = r(mean)*100    // Store the mean

	
	    quietly summarize `var' if hf_type==4 // Calculate mean (proportion of 1s)
    local mean4 = r(mean)*100    // Store the mean

	    quietly summarize `var' if hf_type==5 // Calculate mean (proportion of 1s)
    local mean5 = r(mean)*100    // Store the mean
    ...
    * Write results to Excel
    ...
    putexcel B`row' = `mean1', nformat(0.0) // Write proportion (3 decimal places)
    putexcel c`row' = `mean2', nformat(0.0) // Write proportion (3 decimal places)
    putexcel d`row' = `mean3', nformat(0.0) // Write proportion (3 decimal places)
    putexcel e`row' = `mean4', nformat(0.0) // Write proportion (3 decimal places)
    putexcel f`row' = `mean5', nformat(0.0) // Write proportion (3 decimal places)
...
*% of health facilities that regularly provide each FP method

recode s5_7 s5_7a s5_7b s5_7c s5_7e s5_7f s5_7g s5_7h s5_7j s5_7k s5_7l (2 3 4 5 =0)  (.=0)
lab def yesno 0 no 1 yes,replace

lab val s5_7 s5_7a s5_7b s5_7c s5_7e s5_7f s5_7g s5_7h s5_7j s5_7k s5_7l yesno



lab var s5_7   "Oral contraceptive pills"
lab var  s5_7a "Injectables (any)"
lab var  s5_7b "Male condom"
lab var  s5_7c "Female condom"
lab var  s5_7e "Emergency contraceptive pill"
lab var  s5_7f "IUCD (any)"
lab var  s5_7g "Implants"
lab var  s5_7h "Female sterilization"
lab var  s5_7j "Male sterilization"
lab var  s5_7k "Exclusive breestfeeding"
lab var  s5_7l "Calendar"

egen sac=rowtotal(s5_7 s5_7a s5_7b s5_7c s5_7e s5_7k s5_7l)
egen lac=rowtotal(s5_7f s5_7g s5_7h s5_7j)

gen sac3=sac>2 //At least 3 short-acting methods
gen lac1=lac>0 //At least 1 long-acting method

lab val sac3 lac1 yesno

lab var sac3 "At least 3 short-acting methods"
lab var lac1 "At least 1 long-acting method"

* Define your binary variables
local binary_vars "s5_7 s5_7a s5_7b s5_7c s5_7e s5_7f s5_7g s5_7h s5_7j s5_7k s5_7l sac3 lac1" // Replace with your variable names

*Short-acting methods include: OCP, condom, diaphragm, foam/jelly, injectable, ECP
*Long-acting methods include: IUCD, implants, female and male sterilizations

* Initialize Excel file
putexcel set "HF and regular FP provision.xlsx", sheet("Sheet1") replace

* Write headers
putexcel A1:F1 = "% of health facilities that regularly provide each FP method",bold merge hcenter border(bottom)
putexcel A2 = "Method" B2 = "EPS"  c2 = "CS" d2 = "PS" e2 = "Case de Sante" f2 = "Clinique",bold border(bottom,double)
